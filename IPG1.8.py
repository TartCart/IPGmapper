...
###### Datafile retrieval to be refreshed from time to time (maxmind data) updated 7/10/2023
...
def combine_multiple_input_files(multiple_files):
    ...
    for path in split_list:
        ...
        try:
            ...
            temp_df = pd.read_csv(path, encoding='UTF-16', dtype=str) 
            try:
...
def clean_input_dfs(input_df):
    ...
    with tqdm(total=len(input_df), unit=" Cells analyzed") as pbar:
...
print("\nCleaning all input data and removing redundant IP's, this may take some time...")
